...
LIBRETRO_PCSX_REARMED_CONF_OPTS += -DCMAKE_BUILD_TYPE=Release

define LIBRETRO_PCSX_REARMED_BUILD_CMDS
	$(MAKE) CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" LD="$(TARGET_LD)" RANLIB="$(TARGET_RANLIB)" AR="$(TARGET_AR)" CFLAGS="$(TARGET_CFLAGS)" CXXFLAGS="$(TARGET_CXXFLAGS)" LDFLAGS="$(TARGET_LDFLAGS)" -C $(@D) -f Makefile.libretro
endef
